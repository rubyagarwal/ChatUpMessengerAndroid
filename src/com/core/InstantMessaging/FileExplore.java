			...
			builder.setAdapter(adapter, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					// File picked
                        ...
                        Log.d("File", sel.toString());
                        String path = sel.toString();
                        String[] splits = path.split("/");
                        byte[] msgContents = new MediaChatMessageProcessor().read(path);
                        Payload p = new Payload(MessageType.Contract, Config.TRUE, msgContents, new Timestamp(System.currentTimeMillis()), to, from, splits[splits.length - 1], CallName.Chat);
                        new ChatActivity().sendChatMessage(p, Config.TRUE);
                        Intent i = new Intent(getApplicationContext(), ChatActivity.class);
                        i.putExtra("toUser",to);
                        i.putExtra("signUpUser",from);
                        startActivity(i);
                        finish();
					}
